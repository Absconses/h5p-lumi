...
**How to apply:** après toute modif mémoire/skill, enchaîner automatiquement `Backup-Memory.ps1` puis `git add -A ; git commit ; git push`. Garder les messages de commit en **ASCII** (pas d'accents : PowerShell 5.1 + git les corrompt). Voir [[emi-college-serie]], [[user-profile]].
